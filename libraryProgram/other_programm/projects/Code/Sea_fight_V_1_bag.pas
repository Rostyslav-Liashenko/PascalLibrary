...
function checkMove(i,j,move,palub:integer; s:char):boolean;
begin
  if move = 1 then
    begin
      for var k:=1 to palub do
        begin
          if CheckInp(areaPlayer1,i,j,s) then 
            begin
              result:=true;
            end
          else
      ...
      if move = 2 then
        begin
          for var k:=1 to palub do
            begin
              Inc(j);
              if CheckInp(areaPlayer2,i,j,s) then
                begin
                  result:=true;
                end
              else
...
begin
  ...
  Readln(move);
  if checkInp(z,i,j,s) and CheckMove(i,j,move,k,s) then
    begin
